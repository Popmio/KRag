...
def test_validate_and_apply(neo4j_client, tmp_path):
...
#
#
# def test_drop_missing_does_not_drop_foreign_index(neo4j_client):
#     sm = SchemaManager(neo4j_client)
#
...
#     finally:
#         neo4j_client.execute("DROP INDEX `user_idx_otherx_temp` IF EXISTS")
#
#
# #保护非唯一属性测试
# def test_prune_multilabel_protection_integration(neo4j_client):
#     sm = SchemaManager(neo4j_client)
...
#     neo4j_client.execute("CREATE (n:`A`:`B` {deadprop: 'x', keep: 'y'})")
#     try:
#         # 从 A 的角度移除未声明属性，B 允许的属性应被保护
#         sm.prune_removed_properties(cfg, labels=["A"], dry_run=False)
#
#         # 验证 deadprop 仍然存在（因为节点同时有 B）
#         res = neo4j_client.execute(
#             "MATCH (n:`A`:`B`) RETURN n.deadprop IS NOT NULL AS ok LIMIT 1",
#             readonly=True,
#         )
#         assert res and res[0].get("ok") is True
#     finally:
#         neo4j_client.execute("MATCH (n:`A`:`B`) DETACH DELETE n")
#
#
# @pytest.mark.skipif(True, reason="依赖环境是否支持 VECTOR INDEX，按需启用")
# def test_vector_similarity_normalization_and_drift(neo4j_client):
#     sm = SchemaManager(neo4j_client)
...
#     #     pass
#     finally:
#         neo4j_client.execute("DROP INDEX `vec_vecdrift_emb` IF EXISTS")
